...
public class AddOffer extends FormLayout {
    ...
    public AddOffer(OfferService offerService) {
        ...
        targetPriceField.setAllowedCharPattern("[0-9.]");
        setUserIdFormat(userIdField);
        HorizontalLayout fields = new HorizontalLayout(asin, targetPriceField, currencySelectField, userIdField);
    ...
    private TextField setUserIdFormat(TextField userTextField) {
        userTextField.setAllowedCharPattern("[0-9]");
        int maxSafeLengthForLong = String.valueOf(Long.MAX_VALUE).length() - 1;
        userTextField.setMaxLength(maxSafeLengthForLong);
        return userIdField;
    }
